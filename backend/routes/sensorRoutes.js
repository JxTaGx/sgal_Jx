...
const { sensorValidationRules, handleValidationErrors } = require('../middleware/validators');
...
router.post('/', verifyToken, authorize(['SADMIN', 'ADMIN']), upload.single('fotografia'), sensorValidationRules(), handleValidationErrors, sensorController.createSensor);
...
router.put('/:id', verifyToken, authorize(['SADMIN', 'ADMIN', 'PAP']), upload.single('fotografia'), sensorValidationRules(), handleValidationErrors, sensorController.updateSensor);
